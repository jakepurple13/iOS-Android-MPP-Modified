...
public class BblDialogManager {
    ...
    public static BblContentFragment showBblDialog(FragmentManager fm, LayoutInflater inflater, String content,
                                     String ok, String cancel, String exit, String drawText,
                                     IAlertClickedCallBack alertClickedCallBack, Context context, String TAG, BubbleSetUp setUp) {
        try {
            BblContentFragment fragment = BblContentFragment.newInstance(TAG);
            if (TextUtils.isEmpty(content)) {
                content = context.getString(R.string.err_server_error);
    ...
    public interface BubbleSetUp {
        void setUp(BblContentFragment frag);
    }
}
